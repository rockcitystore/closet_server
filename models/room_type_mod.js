...
const logger = require("../utils/logger").getLogger('room_type_mod');
...
Mod.prototype.list = (data, callback) => {
    return new Promise((resolve, reject) => {
        let tem = [];
        let args = [];
        let sql = [`SELECT ${data.select || '*'} FROM room_type `];
        if (data.room_type_name && data.room_type_name.length > 0) {
            tem.push("room_type_name = ? ");
            args.push(data.room_type_name);
        }
        if (data.room_type_id >= 0) {
            tem.push("room_type_id = ? ");
            args.push(data.room_type_id);
        }
        if (tem.length > 0) {
            sql.push('WHERE ');
        ...
        sql .push(`ORDER BY ${(data.order || "room_type_name")} ${(data.desc || "ASC")} LIMIT ${((data.current - 1) * data.limit) < 0 ? 0 : ((data.current - 1) * data.limit || 0)},${data.limit < 1 ? 1 : (data.limit || 10000)}`);
...
Mod.prototype.add = (data, callback) => {
    return new Promise((resolve, reject) => {
        let args = [data.room_type_name];
        let sql = "insert into room_type (room_type_name) VALUES (?)";
        db.execQuery({
            "sql": sql,
            "args": args,
            "handler": (err, r) => {
...
Mod.prototype.edit = (data, callback) => {
    return new Promise((resolve, reject) => {
        ...
        sql = `${sql}${tem.join(',')} WHERE room_type_id = ? `;
        args.push(data.room_type_id);
        db.execQuery({
            "sql": sql,
            "args": args,
...
Mod.prototype.del = (data, callback) => {
    return new Promise((resolve, reject) => {
        let sql = "DELETE FROM room_type WHERE room_type_id = ?";
        db.execQuery({
            "sql": sql,
            "args": [data.room_type_id],
            "handler": (err, r) => {
                if (err) return reject(err);
                return resolve({code: constant.SUCCESS});
            }
